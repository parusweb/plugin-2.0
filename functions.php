...
// --- Проверка категории 271 (реечные перегородки) ---
function is_partition_slat_category($product_id) {
    $product_categories = wp_get_post_terms($product_id, 'product_cat', ['fields' => 'ids']);
    if (is_wp_error($product_categories) || empty($product_categories)) return false;
    
    foreach ($product_categories as $cat_id) {
        if ($cat_id == 271) return true;
        if (cat_is_ancestor_of(271, $cat_id)) return true;
    }
    return false;
}
...
add_action('wp_footer', function () {
    ...
    $is_partition_slat = is_partition_slat_category($product->get_id());
    ...
    $is_running_meter = has_term([266], 'product_cat', $product->get_id());
    ?>
...
const isPartitionSlat = <?php echo $is_partition_slat ? 'true' : 'false'; ?>;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    <p style="font-weight:600; font-size:1.1em; color:#333; text-align:center; margin-bottom:15px;">Форма верхнего спила:</p>
    <div style="display:flex; gap:10px; justify-content:center;">
        ...
        <svg width="60" height="60" viewBox="0 0 100 100">
    <rect x="0" y="50" width="100" height="50" fill="#8bc34a"/>
    <polygon points="50,0 100,50 0,50" fill="#8bc34a"/>
</svg>
...
console.log('=== Rendering PARTITION SLAT calculator (Category 271) ===');
    ...
    if (selectedService && selectedService.value && typeof paintingServices !== 'undefined') {
        ...
        if (service) {
            ...
            if (selectedColorRadio) {
                colorFileName = selectedColorRadio.getAttribute('data-filename') || '';
                colorImageUrl = selectedColorRadio.value || '';
            }
            
            paintingServiceData = {
                key: serviceKey,
                name: service.name,
                name_with_color: colorFileName ? service.name + ' "' + colorFileName + '"' : service.name,
                price_per_m2: parseFloat(service.price),
                area: totalArea,
                total_cost: paintingCost,
                color_filename: colorFileName,
...
<?php if($is_multiplier && !$is_running_meter && !$is_partition_slat && !$show_falsebalk_calc): ?>
...
if (typeof resultBlock !== 'undefined' && resultBlock) {
    ...
    console.error('❌ resultBlock not found for multiplier calc');
    const form = document.querySelector('form.cart');
    if (form) {
        form.insertAdjacentElement('afterend', multiplierCalc);
    }
}
...
if (showThickness) { html += `Толщина: <b>40мм</b><br>`; }
    html += `Цена за 1 шт: <b>${(pricePerItem + shapeExtraPerItem).toFixed(2)} ₽</b><br>`;
    html += `<br>`;
    html += `Стоимость материала: <b>${(materialPrice + shapeExtraTotal).toFixed(2)} ₽</b><br>`;
...
if (typeof resultBlock !== 'undefined' && resultBlock) {
    ...
    console.error('❌ resultBlock not found for running meter calc');
    const form = document.querySelector('form.cart');
    if (form) {
        form.insertAdjacentElement('afterend', runningMeterCalc);
    }
}
...
if (typeof resultBlock !== 'undefined' && resultBlock) {
    ...
    console.error('❌ resultBlock not found for square meter calc');
    const form = document.querySelector('form.cart');
    if (form) {
        form.insertAdjacentElement('afterend', sqMeterCalc);
    }
}
...
function add_category_faska_fields($term) {
    $allowed = [273, 281]; // ID категорий, где показывать поля
    if (!in_array($term->term_id, $allowed)) return;

    $term_id = $term->term_id;
    $faska_types = get_term_meta($term_id, 'faska_types', true);
    if (!$faska_types) $faska_types = [];
...
/* === Интеграция верхушки с множителем калькулятора === */
...
add_action('wp_footer', function() {
    ...
    // Проверяем категорию штакетника (273)
    if (!has_term(273, 'product_cat', $product_id)) return;
    
    // Получаем цены верхушек из админки
    $shape_price_round = floatval(get_post_meta($product_id, '_shape_price_round', true)) ?: 0;
    $shape_price_triangle = floatval(get_post_meta($product_id, '_shape_price_triangle', true)) ?: 0;
    $shape_price_flat = floatval(get_post_meta($product_id, '_shape_price_flat', true)) ?: 0;
    
    ?>
    <script>
    // === ГЛОБАЛЬНАЯ ПЕРЕМЕННАЯ с ценами верхушек ===
    // Это должна быть ПЕРВАЯ строка в скрипте, чтобы была доступна везде
    window.shapePrices = {
        'round': <?php echo $shape_price_round; ?>,
        'triangle': <?php echo $shape_price_triangle; ?>,
        'flat': <?php echo $shape_price_flat; ?>
    };
    

    (function() {
        function initShapeListeners() {
            // Находим форму
            const form = document.querySelector('form.cart');
            if (!form) {
                setTimeout(initShapeListeners, 500);
                return;
            }
            
            // Ищем input[name="shape_type"]
            let shapeInputs = form.querySelectorAll('input[name="shape_type"]');
            if (shapeInputs.length === 0) {
                shapeInputs = document.querySelectorAll('input[name="shape_type"]');
            }
            ...
            // Находим функцию обновления калькулятора (она уже определена на странице)
            if (typeof updateMultiplierCalc === 'undefined') {
                console.warn('⚠️ updateMultiplierCalc not found, retrying...');
                setTimeout(initShapeListeners, 500);
                return;
            }
            

            // СЛУШАЕМ КЛИКИ ПО ВЕРХУШКЕ
            shapeInputs.forEach(input => {
                // Слушаем change
                input.addEventListener('change', function() {
                ...
                // Слушаем клик по лейблу (для SVG)
                const label = input.closest('label');
                if (label) {
                    label.addEventListener('click', function() {
                        setTimeout(updateMultiplierCalc, 100);
                    });
                }
            });
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', initShapeListeners);
        } else {
            initShapeListeners();
        }
        
        // Повторяем через время на случай асинхронной загрузки
        setTimeout(initShapeListeners, 1000);
    })();
    ...
    <?php
}, 15); // Меньше приоритет, чтобы запустилось ПЕРЕД калькулятором множителя



// Добавляем поля цен для форм верха штакетника
add_action('woocommerce_product_options_pricing', function() {
    global $post;
    
    // Проверяем, относится ли товар к категории штакетника (273)
    ...
    woocommerce_wp_text_input([
        ...
        'label' => 'Цена "Полукруг" (₽)',
        'desc_tip' => true,
        'description' => 'Дополнительная цена за полукруглую форму верха штакетника',
        'type' => 'number',
        'custom_attributes' => [
            'step' => '0.01',
            'min' => '0'
        ],
        'value' => get_post_meta($post->ID, '_shape_price_round', true)
    ]);
    ...
    woocommerce_wp_text_input([
        ...
        'desc_tip' => true,
        'description' => 'Дополнительная цена за треугольную форму верха штакетника',
        'type' => 'number',
        'custom_attributes' => [
            'step' => '0.01',
            'min' => '0'
        ],
        'value' => get_post_meta($post->ID, '_shape_price_triangle', true)
    ]);
    ...
    woocommerce_wp_text_input([
        ...
        'desc_tip' => true,
        'description' => 'Дополнительная цена за прямую форму верха штакетника',
        'type' => 'number',
        'custom_attributes' => [
            'step' => '0.01',
            'min' => '0'
        ],
        'value' => get_post_meta($post->ID, '_shape_price_flat', true)
    ]);
    
    echo '</div>';
});

// Сохраняем значения полей при сохранении товара
add_action('woocommerce_process_product_meta', function($post_id) {
    // Проверяем категорию
    ...
    // Сохраняем цены форм
    $shape_fields = ['_shape_price_round', '_shape_price_triangle', '_shape_price_flat'];
...
(function() {
    ...
    function setupCalculatorFieldListeners() {
        ...
        // Отслеживание цвета, фаски, формы верха
        document.addEventListener('change', function(e) {
            if (e.target.name === 'pm_selected_color' || 
